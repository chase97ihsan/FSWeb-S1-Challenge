...
function indekseGoreFenomen(dizi,index) {
  let a= dizi[index].profile
  return index+"."+ " indekste bulunan fenomen: " + a
...
*/
function profilListesi(dizi2) {
  let dizi6=[...dizi2];

  for (let i = 0; i < dizi6.length; i++) {
    dizi6[i]=dizi6[i].profile
...
function fenomenSil(dizi3,index3) {
  let diizi = [];
  diizi=dizi3;
  diizi.splice(index3,1);
return diizi
...
function fenomenEkle(dizi4,number,profile,followers,posts,platform) {
  var dizi5=[];
  dizi5=dizi4;
  let nesnee={
    "number": number,
...
function enFenomenler(fenomenler) {
  ...
  for (let index = 0; index < fenomenler.length; index++) {
    if(fenomenler[index].followers>100000000){
    result.push(fenomenler[index].profile);
...
function fenomenGonderimSayisi(fenomenler,ad){
for (let index = 0; index < fenomenler.length; index++) {
  if(fenomenler[index].profile==ad){
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,yer){
let w=[];

  for (let index = 0; index <fenomenler.length; index++) {
  if(fenomenler[index].platform==yer)
w.push(fenomenler[index].posts);
} 
 let q=Math.max(...w);
 for (let index = 0; index < fenomenler.length; index++) {
  if(q==fenomenler[index].posts){
    return  fenomenler[index].profile
  }
